...
This is a replication of Experiment 1, and we swap the ground truth causal power of stripes and spots as from Experiment 1, in order to confirm that the learning effect showed in Experiment 1 is not tied to the particular stimuli.
...
- **Ground-truth** rule is a mixture of multiplication and subtraction: `Blocks(R') <- Dots(A) * Blocks(R) - Stripes(A)`.

And three **learning conditions**:

1. `Construct`: First build the `Dots(A) * Blocks(R)` sub-part, and then re-use it to build the ground truth rule
  ![](../figs/exp_2/construct.png)
  
2. `Deconstruct`: reverse order of the `construct` condition
  ![](../figs/exp_2/decon.png)
  
3. `Combine`: First build the `Dots(A) * Blocks(R)` sub-part, and then build the `- Stripes(A)` sub-part
...
Pre-registration: <https://osf.io/k5dc3>


# Pilot results


Recruited N = 165 participants on Prolific (age = `r round(mean(df.sw$age))` ± `r round(sd(df.sw$age),1)`). 
...
for (cond in c('construct', 'combine', 'decon')) {
  for (ph in c('a', 'b')) {
    preds = read.csv(paste0('../data/model_preds/exp_2/', cond, '_preds_', ph, '.csv'))
    preds_fmt = preds %>%
