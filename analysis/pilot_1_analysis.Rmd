...
![](figs/pilot_1_raws_labeled.png)

# Model predictions

Below is a very rough model prediction sketch, on the `combined` condition, with just 100 iterations.
It seems the model can roughly catch similar prediction trend as humans, but way less noisy (in each trial the model's uncertainties are centered around two options) and shows little difference between phase A and B (the programs learned in phase A overwhelms phase B?).
