...
g1.sample_hypos(all_frames, top_n=1, save_prefix='samples/combine_a_sam')


# Gen predictions A
a_ppl = Task_lib(g1.cur_programs)
a_gen = sim_for_all(task_data['gen'], a_ppl, 5)
a_gen.to_csv('combine_sampled_preds_a.csv')


# Learning phase B
g2 = Task_gibbs(Task_lib(g1.cur_programs), task_data['learn_b'], iteration=500)
g2.sample_hypos(all_frames, top_n=1, save_prefix='samples/combine_b_sam')


# Gen predictions B
b_ppl = Task_lib(g2.cur_programs)
b_gen = sim_for_all(task_data['gen'], b_ppl, 1000)
b_gen.to_csv('combine_sampled_preds_b.csv')
